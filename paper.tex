...
the publicly released \texttt{daily\_news} collection.\footnote{Data accessed
from the Hugging Face Hub (\texttt{TheFinAI/daily\_news}). The competition-venue
metadata (CLEF~2026) should be confirmed by the authors prior to
camera-ready.} It comprises twelve assets---ten US equities (AAPL, ADBE, AMZN,
...
$q$-th percentile of $|r|$ over the training portion (first 70\%) of each asset:
\begin{equation}
\tau_a = \mathrm{P}_{q}\big(\{|r_{s,a}| : s \in \text{train}(a)\}\big),
\qquad q = 33,
\end{equation}
...
same seed reaches a Sharpe of $2.88$. These are the operating points selected on
validation; Section~\ref{sec:seed} reports the across-seed distribution and
should be read alongside this table. Figure~\ref{fig:equity} shows the
out-of-sample equity curves: the SWA model is more selective (it opens fewer
positions at the higher tuned threshold) and produces a smoother trajectory in
the second half of the test window.
...
\begin{table}[t]
\centering
\caption{Test Sharpe ratio across five seeds of the deployed configuration.
The deployed checkpoint is the validation-selected seed (highest validation
Sharpe). Aggregate statistics use the sample standard deviation.}
\label{tab:seed}
\begin{tabular}{lccccc}
\toprule
Variant & mean SR & std & min & max & deployed (val-selected) \\
\midrule
Base (no SWA) & $+1.70$ & $1.75$ & $-0.22$ & $+3.28$ & $+2.88$ \\
+ SWA         & $+0.75$ & $1.95$ & $-0.49$ & $+4.21$ & $\mathbf{+4.21}$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{credits}
\noindent\textbf{Acknowledgments.} [TO BE COMPLETED.]
