...
\paragraph{Weekend and holiday consolidation.}
For equities, weekend and holiday records are removed because US markets do
not operate on those days. Their associated news are carried forward to the
next trading day with an exponential temporal decay. The carry-forward weight
is $w = \exp(-\lambda\,\Delta_{\text{days}})$ where $\lambda = 0.18$ and
$\Delta_{\text{days}}$ counts only the number of non-trading days between the
original and the target date. For example, Friday news moved to Monday have
$\Delta_{\text{days}} = 2$ (Saturday and Sunday), yielding
$w = \exp(-0.18 \times 2) \approx 0.70$; the same logic applies to multi-day
holiday breaks. Cryptocurrencies trade seven days a week and therefore require
no consolidation.
...
class counts reported in Table~\ref{tab:dataset}.
...
With the features and labels defined, the data is split temporally per asset
into train ($65\%$), validation ($15\%$), and test ($20\%$) partitions. The
per-asset splits are then concatenated, preserving chronological order and
...
ones, making it less sensitive to outliers than mean squared error.
